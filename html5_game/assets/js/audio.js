var audiogame={};
audiogame.dots = [];
audiogame.isRecordMode = false;   // 紀錄音樂資料模式
...
$(function(){
	...
	audiogame.btnOverSound = document.getElementById("btn_over");
	audiogame.btnOverSound.volume = .7;
	audiogame.btnActiveSound = document.getElementById("btn_active");
	audiogame.btnOverSound.volume = .7;
	audiogame.dotImage.src="assets/img/audio/dot.png";
	
	
	
	$("a[href='#game']").hover(function(){  //移過去開始撥
	...
	$(document).keydown(function(e){
		...
		if(audiogame.isRecordMode ){
			...
			//紀錄
			else if (e.which ==74 || e.which ==75 || e.which ==76){
			...
			//our target us J(74),K(75),L(76)
			//check if hit a music note dol
			var hitline = e.which-73;
			for(var i in audiogame.dots){
				if(line== audiogame.dots[i].line && Math.abs(audiogame.dots[i].distance)<20){
					//remove the hit dot from the dots array
					audiogame.dots.splice(i,1);
					//increase the sucess count
...
function onMelodyEnded(){
	...
	console.log('Precision ' + audiogame.totalSuccessCount/audiogame.totalDotsCount * 100 +"%");
	$("#results").html('Precision ' + audiogame.totalSuccessCount/audiogame.totalDotsCount * 100 +"%");
}
...
function gameloop(){
	...
	var sucessPercent = audiogame.successCount /5;
	sucessPercent=Math.max(0,Math.min(1,sucessPercent));
	audiogame.melody.volume=sucessPercent;
	// move the dots
	for(var i in audiogame.dots){
